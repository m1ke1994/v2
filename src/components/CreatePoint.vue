<!-- 
<script>
import { usePointsStore } from '@/stores/pointsStore'; // Путь к вашему хранилищу Pinia
...
export default {
    ...
    methods: {
        savePoint() {
            ...
            const timeBefore = new Date(this.timeBefore);
            const timeLoading = new Date(this.timeLoading);
            const timeAfter = new Date(this.timeAfter);
            const timeBeforeFact = new Date(this.timeBeforeFact);
            const timeAfterFact = new Date(this.timeAfterFact);
            ...
            const point = {
                ...
                r: isNaN(r) ? 0 : r,
                codpoints: isNaN(codpoints) ? 0 : codpoints,
                selected: this.selected,
                timeBefore: isNaN(timeBefore.getTime()) ? null : timeBefore,
                timeLoading: isNaN(timeLoading.getTime()) ? null : timeLoading,
                timeAfter: isNaN(timeAfter.getTime()) ? null : timeAfter,
                timeBeforeFact: isNaN(timeBeforeFact.getTime()) ? null : timeBeforeFact,
                timeAfterFact: isNaN(timeAfterFact.getTime()) ? null : timeAfterFact,
            };
            ...
            <div class="w-3/6 pr-4">
                ...
                <ul class="list-none text-neutral-500">
                    <li>
                        ...
                        <input v-model="title" type="text" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Код точки</label>
                        <input v-model="codpoints" type="text" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Широта</label>
                        <input v-model="x" type="text" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Долгота</label>
                        <input v-model="y" type="text" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Радиус</label>
                        <input v-model="r" type="text" class="w-full border p-2">
                    </li>
                </ul>
            </div>
            <div class="w-3/6 pl-4">
                ...
                <ul class="list-none text-neutral-500">
                    <li>
                        ...
                        <input v-model="timeBefore" type="datetime-local" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Время прибытия по:</label>
                        <input v-model="timeAfter" type="datetime-local" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Время загрузки:</label>
                        <input v-model="timeLoading" type="datetime-local" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Время прибытия (факт):</label>
                        <input v-model="timeBeforeFact" type="datetime-local" class="w-full border p-2">
                    </li>
                    <li>
                        <label>Время убытия (факт):</label>
                        <input v-model="timeAfterFact" type="datetime-local" class="w-full border p-2">
                    </li>
                </ul>
            </div>
        </div>
